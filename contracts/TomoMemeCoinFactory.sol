...
contract TomoMemeCoinFactory is OwnableUpgradeable {
    ...
    DataTypes.CreateTomoMemeCoinParameters private _parameters;
    address public _tomoMemeCoinManager;
    ...
    function initialize(
        address owner,
        address tomoMemeCoinManager
    ) public initializer {
        if (owner == address(0) || tomoMemeCoinManager == address(0)) {
            revert ZeroAddress();
        }
        __Ownable_init(owner);

        _tomoMemeCoinManager = tomoMemeCoinManager;
    }

    function createTomoMemeCoin(
        DataTypes.CreateTomoMemeCoinParameters calldata vars
        ...
        ITomoMemeCoinManager(_tomoMemeCoinManager).prePairTomoMemeCoinEnv(
            memeCoin,
            vars.sqrtPriceX96,
            vars.sqrtPriceB96
        );
        ...
        _emitCreateTomoMemeCoinEvent(memeCoin, vars);
    }

    function setTomoMemeCoinManager(address newAddr) public onlyOwner {
        if (newAddr == address(0)) {
            revert ZeroAddress();
        }
        _tomoMemeCoinManager = newAddr;
    }
    ...
    function _checkParam(
        ...
        (
            bool _canCreateTomoMemeCoin,
            uint256 _maxPurchasePercentageForCreator,
            uint256 _maxPreSaleTime
        ) = ITomoMemeCoinManager(_tomoMemeCoinManager)
                .getCreatTomoMemeCoinParam();

        if (!_canCreateTomoMemeCoin) {
            revert CantCreateTomoMemeCoin();
        }
        if (
    ...
    function _emitCreateTomoMemeCoinEvent(
        address memeCoin,
        DataTypes.CreateTomoMemeCoinParameters calldata vars
    ) internal {
        emit TomoMemeCoinCreated(
            memeCoin,
            vars.creator,
            vars.totalSupply,
            vars.reserved,
