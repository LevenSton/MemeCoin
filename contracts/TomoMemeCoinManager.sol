...
import {ITomoMemeCoin} from "./interfaces/ITomoMemeCoin.sol";
import {IWETH9} from "./interfaces/IWETH9.sol";
import {ITomoMemeCoinFactory} from "./interfaces/ITomoMemeCoinFactory.sol";
import {TransferHelper} from "./libraries/TransferHelper.sol";

contract TomoMemeCoinManager is ITomoMemeCoinManager {
    ...
    bool public _canCreateTomoMemeCoin;
    ...
    function addLiquidityForTomoMemeCoin(
        ...
        if (msg.sender != tomoMemeCoinAddr) {
            revert OnlyCallByTomoMemeCoin();
        }
        ...
        uint256 leftToken = ITomoMemeCoin(tomoMemeCoinAddr).balanceOf(
            address(this)
        );
        address creator = ITomoMemeCoin(tomoMemeCoinAddr).creator();
        if (leftToken > 0) {
            TransferHelper.safeTransfer(tomoMemeCoinAddr, creator, leftToken);
        }
    ...
    function collect(
        ...
        uint256 tokenReward = ITomoMemeCoin(tomoMemeCoinAddr).balanceOf(
            address(this)
        );
    ...
    function setCreateTomoMemeCoin(bool canCreate) public onlyOwner {
        if (_canCreateTomoMemeCoin == canCreate) {
            revert InvaildParam();
        }
        _canCreateTomoMemeCoin = canCreate;
    }
    ...
    function _mintLiquidity(
        ...
        (address token0, address token1, bool zeroForOne) = tomoMemeCoinAddr <
            _weth
            ? (tomoMemeCoinAddr, _weth, true)
            : (_weth, tomoMemeCoinAddr, false);
