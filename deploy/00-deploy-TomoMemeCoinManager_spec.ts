...
const deployFn: DeployFunction = async (hre) => {
  ...
  const proxyTomoMemeCoinFactoryNonce = hexlify(toBeHex((deployerNonce + 2).toString()));
  const proxyAddress = '0x' + keccak256(encode([deployer, proxyTomoMemeCoinFactoryNonce])).substring(26);
  ...
  await deployAndVerifyAndThen({
    hre,
    name: "TomoMemeCoinManager",
    contract: 'TomoMemeCoinManager',
